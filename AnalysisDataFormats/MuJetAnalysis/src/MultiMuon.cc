//
// $Id: MultiMuon.cc,v 1.11 2010/12/09 11:38:17 pivarski Exp $
// 
...
/// default constructor
pat::MultiMuon::MultiMuon(std::vector<const pat::Muon*> &muons, const TransientTrackBuilder *transientTrackBuilder, const reco::TrackCollection *tracks, const pat::MuonCollection *allmuons, const CaloTowerCollection *caloTowers, double centralTrackIsolationCone, double unionTrackIsolationCone, double centralTrackThresholdPt, double unionTrackThresholdPt, double centralCaloIsolationCone, double unionCaloIsolationCone, double centralNumberAboveThresholdCone, double unionNumberAboveThresholdCone, double centralNumberAboveThresholdPt, double unionNumberAboveThresholdPt) {
...
//       throw cms::Exception("MultiMuon") << "All muons must have an innerTrack (globalMuon or trackerMuon)";
//     }
////// 
      addDaughter(**muon);
      charge += (*muon)->charge();
   ...
   m_chi2 = 0.;
   m_ndof = 0.;
   if (transientTrackBuilder != NULL) calculateVertex(transientTrackBuilder);
   ...
   if (tracks != NULL) calculateTrackIsolation(tracks, allmuons, centralTrackIsolationCone, unionTrackIsolationCone, centralTrackThresholdPt, unionTrackThresholdPt);
   ...
   if (caloTowers != NULL) calculateCaloIsolation(caloTowers, centralCaloIsolationCone, unionCaloIsolationCone);
   ...
   if (tracks != NULL) calculateNumberAboveThresholdIsolation(tracks, allmuons, centralNumberAboveThresholdCone, unionNumberAboveThresholdCone, centralNumberAboveThresholdPt, unionNumberAboveThresholdPt);
}
...
//    std::cout << std::endl << "constructor from MultiMuonType" << std::endl;
//    std::cout << "m_chi2: " << m_chi2 << std::endl;
//    std::cout << "m_ndof: " << m_ndof << std::endl;
//    std::cout << "m_covarianceMatrix: " << m_covarianceMatrix << std::endl;
...
//      update(const CachingVertex &fittedVertex, RefCountedVertexTrack tracksToVertex[0])

//    typedef ReferenceCountingPointer<CachingVertex<5> > RefCountedVertexTrack;
//    typedef ReferenceCountingPointer<VertexTrack<5> > RefCountedVertexTrack;

// //    std::vector<reco::Track> newTracks = fittedVertex.tracks();
//    std::vector<RefCountedVertexTrack> newTracks = fittedVertex.tracks();


//    for (unsigned int i = 0; i < newTracks.size(); i++){
//      std::cout <<"newTracks[i]: " << (newTracks)[i] << std::endl;
//    }
...
//      }
   ...
   double covarianceMatrixArray[6] = {fittedVertex.error().cxx(), fittedVertex.error().cyx(), fittedVertex.error().cyy(), fittedVertex.error().czx(), fittedVertex.error().czy(), fittedVertex.error().czz() };
...
void pat::MultiMuon::calculateTrackIsolation(const reco::TrackCollection *tracks, const pat::MuonCollection *allmuons, double centralCone, double unionCone, double centralThreshold, double unionThreshold, TTree *diagnosticTTree, Float_t *diagnosticdR, Float_t *diagnosticpT) {
...
void pat::MultiMuon::calculateNumberAboveThresholdIsolation(const reco::TrackCollection *tracks, const pat::MuonCollection *allmuons, double centralCone, double unionCone, double centralThreshold, double unionThreshold, TTree *diagnosticTTree, Float_t *diagnosticdR, Float_t *diagnosticpT) {
...
pat::MultiMuon pat::MultiMuon::merge(const pat::MultiMuon &aMultiMuon, const TransientTrackBuilder *transientTrackBuilder, const reco::TrackCollection *tracks, const pat::MuonCollection *allmuons, const CaloTowerCollection *caloTowers, double centralTrackIsolationCone, double unionTrackIsolationCone, double centralTrackThresholdPt, double unionTrackThresholdPt, double centralCaloIsolationCone, double unionCaloIsolationCone, double centralNumberAboveThresholdCone, double unionNumberAboveThresholdCone, double centralNumberAboveThresholdPt, double unionNumberAboveThresholdPt) {
